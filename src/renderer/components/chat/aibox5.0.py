...
# ========= 路径设置 =========
# 兼容开发环境和PyInstaller打包后的路径
def resource_path(relative_path):
    """ 获取资源的绝对路径，支持PyInstaller打包 """
    try:
        # PyInstaller会把临时资源放在_sys._MEIPASS里
...
# 你原本的资源目录，改成相对 BASE_DIR
COMMON_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "common"))
PHOTO_DIR = os.path.abspath(os.path.join(BASE_DIR, "photo"))

# PyInstaller打包时要用resource_path加载资源
TOKEN_PATH       = resource_path(os.path.join(COMMON_DIR, "token.txt"))
EMOTION_FILE     = resource_path(os.path.join(COMMON_DIR, "emotion_tag.txt"))
CHARACTER_PATH   = resource_path(os.path.join(COMMON_DIR, "character.txt"))
AVATAR_PATH = lambda persona: resource_path(os.path.join(PHOTO_DIR, f"{persona}.png"))
USER_AVATAR_PATH = resource_path(os.path.join(PHOTO_DIR, "mxr.png"))
ARROW_IMG_PATH   = resource_path(os.path.join(PHOTO_DIR, "arrow.png"))
