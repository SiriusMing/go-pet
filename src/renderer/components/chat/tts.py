...
APPID = '01403d4d'
APISECRET = 'YjFjM2NjODU0ZjI5Njk2YWY5MzU4MWZh'
APIKEY = '0723dbeee57dcaa91a43a3bc69c65a71'
...
def text2audio2(text, vcn):
    wsParam = Ws_Param(APPID='01403d4d', APISecret='YjFjM2NjODU0ZjI5Njk2YWY5MzU4MWZh',
                       APIKey='0723dbeee57dcaa91a43a3bc69c65a71',
                       Text=text, VCN=vcn)
...
def text2audio(text, vcn):
    ...
    ws = websocket.WebSocketApp(wsUrl,
                                 ...
                                 on_error=on_error,
                                 on_close=on_close)
    ws.on_open = on_open_factory(wsParam)

    ws_thread = threading.Thread(target=ws.run_forever, kwargs={"sslopt": {"cert_reqs": ssl.CERT_NONE}})
    ws_thread.start()

    print("等待音频接收完成...")
    audio_received.wait(timeout=15)  # 最多等待 15 秒

    if os.path.exists("demo.pcm"):
        pcm_to_wav("demo.pcm", "demo.wav")
        play_wav("demo.wav")
    else:
        print("demo.pcm 不存在，转换失败")
